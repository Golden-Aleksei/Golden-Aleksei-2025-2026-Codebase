...
@Disabled // So that Andrea does not touch this or someone else runs this
@Configurable
@TeleOp(name = "Programmer TeleOp")
public class ProgrammerTeleOp extends OpMode {
    ...
    private int OuttakeRunning = 0;
    private int ServoRunning = 0;
    private int LightRunning = 0;
    ...
    public void loop() {
        ...
        if (gamepad1.xWasPressed()) {
            if ((headingError > 0.1) || (headingError < -0.1)) { // Not 0 for reason
        ...
        if (result != null && result.isValid()) {
            ...
            if ((10 >= headingError) && (headingError>= -10)){ // Probably will have to be different values
